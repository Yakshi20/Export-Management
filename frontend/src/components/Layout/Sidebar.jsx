...
export default function Sidebar({ links }) {
  ...
  const displayName = (role === 'exporter' ? user?.companyName : role === 'farmer' ? user?.farmerName : user?.name) || user?.companyName || user?.farmerName || user?.name || emailName || 'User';
  ...
  const openDrawer = async () => {
    setDrawerOpen(true); setTab('profile');
    try { const res = await api.get(`/${role}/profile`); setProfileData(res.data?.data || res.data); } catch {}
  };
  ...
  const saveName = async () => {
    ...
    try {
      ...
      const updated = { ...user, [nameField]: newName.trim() };
    ...
    } catch (e) { showMsg(e?.response?.data?.message || 'Failed to update name', 'error'); }
    setSaving(false);
  };
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      login(localStorage.getItem('token'), { ...user, photo: ev.target.result });
  ...
  const tabs = [{ id: 'profile', label: '👤 Info' }, { id: 'name', label: '✏️ Name' }, { id: 'password', label: '🔒 Password' }];
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-30 flex items-center px-4 py-3 bg-[#16213e] border-b border-white/10">
        ...
        <div className="flex-1 text-center"><p className="text-white text-sm font-semibold">Hello, {displayName}</p></div>
      ...
      <div className={`fixed top-0 left-0 z-50 h-full w-72 bg-[#16213e] border-r border-white/10 shadow-2xl transition-transform duration-300 flex flex-col ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {msg && <div className={`mb-3 px-3 py-2 rounded-lg text-xs font-medium ${msg.type === 'error' ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>{msg.text}</div>}
          {tab === 'profile' && (
          ...
          {tab === 'password' && (
            <div className="space-y-3">
              {['current', 'next', 'confirm'].map((field, i) => (
                <div key={field}>
                  <label className="text-[#a8b2d8] text-xs block mb-1">{i === 0 ? 'Current Password' : i === 1 ? 'New Password' : 'Confirm Password'}</label>
        ...
        <div className="px-5 pb-6">
          <button onClick={handleLogout} className="w-full py-2.5 rounded-xl bg-red-500/10 border border-red-400/30 text-red-400 font-semibold text-sm hover:bg-red-500/20 transition-all">
            🚪 Logout
          </button>
        </div>
      </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 bg-[#16213e] border-t border-white/10 flex items-center justify-around px-2 py-2">
        {links.map(link => (
          <NavLink key={link.path} to={link.path} end={link.end}
            className={({ isActive }) => `flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-all duration-200 text-xs font-medium min-w-[56px] ${isActive ? 'text-[#6366f1] bg-[#6366f1]/10' : 'text-[#a8b2d8] hover:text-white'}`}>
