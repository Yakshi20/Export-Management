...
export default function RegisterPage() {
  ...
  const inp = "w-full border border-[#d0d5ee] rounded-xl px-4 py-2.5 text-[#1a1a2e] focus:outline-none focus:border-[#6366f1] bg-white text-sm";
  return (
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-[#e0e4f0] p-8">
          ...
          <form onSubmit={register} className="space-y-4">
            <div><label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Email</label>
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="your@email.com" required className={inp} /></div>
            <div><label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Mobile</label>
              <input value={form.mobile} onChange={e => setForm({ ...form, mobile: e.target.value })} placeholder="+91 9999999999" required className={inp} /></div>
            <div><label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Password</label>
              <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="........" required className={inp} /></div>
            <div><label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Re-enter Password</label>
              <input type="password" value={form.confirmPassword} onChange={e => setForm({ ...form, confirmPassword: e.target.value })} placeholder="........" required className={inp} /></div>
            {extraFields.map(f => (
              <div key={f.name}><label className="text-sm font-medium text-[#1a1a2e] mb-1 block">{f.label}</label>
                <input type={f.type || 'text'} value={form[f.name] || ''} onChange={e => setForm({ ...form, [f.name]: e.target.value })} required className={inp} /></div>
            ))}
            <button type="submit" disabled={loading} className="w-full py-3 bg-[#6366f1] text-white font-bold rounded-xl hover:bg-[#5254cc] transition-all disabled:opacity-60 mt-2">
