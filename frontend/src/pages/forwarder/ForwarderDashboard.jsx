...
function QuotesPage() {
  ...
  const calculate = () => { const w = Number(form.weight) || 0; const isAir = form.type === 'air'; const freight = isAir ? w * 180 : w * 45; const customs = Math.round(freight * 0.08); const insurance = Math.round(freight * 0.02); const localTransport = isAir ? 3500 : 5000; setQuote({ freight, customs, insurance, localTransport, total: freight + customs + insurance + localTransport }); };
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3"><div><label className="text-[#a8b2d8] text-xs mb-1 block">Weight (kg)</label><input type="number" value={form.weight} onChange={e => setForm(f => ({ ...f, weight: e.target.value }))} placeholder="500" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6366f1]" /></div><div><label className="text-[#a8b2d8] text-xs mb-1 block">Freight Type</label><select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6366f1]"><option value="sea" className="bg-[#16213e]">🚢 Sea Freight</option><option value="air" className="bg-[#16213e]">✈️ Air Freight</option></select></div></div>
          <button onClick={calculate} className="w-full py-2.5 bg-[#6366f1] text-white font-bold rounded-xl hover:bg-[#5254cc] transition-all">Calculate Quote</button>
        </div>
        {quote && (<div className="mt-4 border-t border-white/10 pt-4 space-y-2"><p className="text-xs text-[#6366f1] font-semibold uppercase tracking-widest mb-2">Estimated Costs</p>{[[`${form.type==='air'?'✈️ Air':'🚢 Sea'} Freight`,`₹${quote.freight.toLocaleString()}`],['🛃 Customs (8%)',`₹${quote.customs.toLocaleString()}`],['🛡 Insurance (2%)',`₹${quote.insurance.toLocaleString()}`],['🚛 Local Transport',`₹${quote.localTransport.toLocaleString()}`]].map(([k,v])=>(<div key={k} className="flex justify-between text-sm border-b border-white/5 pb-2"><span className="text-[#a8b2d8]">{k}</span><span className="text-white">{v}</span></div>))}<div className="flex justify-between text-sm pt-1"><span className="text-white font-bold">Total</span><span className="text-green-400 font-bold text-lg">₹{quote.total.toLocaleString()}</span></div></div>)}
      </Card>
      <h2 className="text-lg font-bold text-white">Quote Requests</h2>
      <div className="space-y-3">{quotes.map(q => (<Card key={q.id}><div className="flex items-start justify-between gap-3 mb-3"><div><p className="text-white font-bold text-sm">{q.exporter}</p><p className="text-[#a8b2d8] text-xs">{q.route} · {q.weight}</p></div><span className={`text-xs px-2 py-1 rounded-full font-semibold ${q.status==='accepted'?'bg-green-400/10 text-green-400':'bg-yellow-400/10 text-yellow-400'}`}>{q.status}</span></div><div className="grid grid-cols-2 gap-3 mb-3"><div className="bg-white/5 rounded-lg p-2 text-center"><p className="text-xs text-[#a8b2d8]">✈️ Air</p><p className="text-white font-bold">₹{q.air.toLocaleString()}</p></div><div className="bg-white/5 rounded-lg p-2 text-center"><p className="text-xs text-[#a8b2d8]">🚢 Sea</p><p className="text-white font-bold">₹{q.sea.toLocaleString()}</p></div></div>{q.status==='pending'&&<button onClick={()=>setQuotes(list=>list.map(x=>x.id===q.id?{...x,status:'accepted'}:x))} className="w-full py-1.5 bg-[#6366f1] text-white text-sm font-bold rounded-xl">Send Quote</button>}</Card>))}</div>
    </div>
  );
}

function DocumentsPage() {
  const [docs, setDocs] = useState([{ id:1,shipment:'Mumbai → Dubai (Ravi Exports)',type:'Bill of Lading',uploaded:false,file:null },{ id:2,shipment:'Delhi → London (Sun Traders)',type:'Airway Bill',uploaded:true,file:'AWB-20240112.pdf' },{ id:3,shipment:'Mumbai → Dubai (Ravi Exports)',type:'Shipping Invoice',uploaded:false,file:null },{ id:4,shipment:'Chennai → Singapore (Blue Ocean)',type:'Customs Clearance',uploaded:true,file:'CC-20240110.pdf' },{ id:5,shipment:'Delhi → London (Sun Traders)',type:'Packing List',uploaded:false,file:null }]);
  const upload = (id) => setDocs(d => d.map(doc => doc.id===id?{...doc,uploaded:true,file:`${doc.type.replace(/ /g,'-')}-${Date.now()}.pdf`}:doc));
  const DOC_ICONS = {'Bill of Lading':'🚢','Airway Bill':'✈️','Shipping Invoice':'🧾','Customs Clearance':'🛃','Packing List':'📋'};
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-white">📄 Document Management</h1>
      <p className="text-[#a8b2d8] text-sm">Upload and manage shipping documents for each shipment.</p>
      <div className="grid grid-cols-3 gap-3">{[{label:'Total',value:docs.length,color:'text-white'},{label:'Uploaded',value:docs.filter(d=>d.uploaded).length,color:'text-green-400'},{label:'Pending',value:docs.filter(d=>!d.uploaded).length,color:'text-yellow-400'}].map(s=>(<Card key={s.label} className="text-center py-2"><p className={`text-xl font-bold ${s.color}`}>{s.value}</p><p className="text-[#a8b2d8] text-xs">{s.label}</p></Card>))}</div>
      <div className="space-y-3">{docs.map(d=>(<Card key={d.id}><div className="flex items-start gap-3"><span className="text-2xl">{DOC_ICONS[d.type]||'📄'}</span><div className="flex-1"><p className="text-white font-bold text-sm">{d.type}</p><p className="text-[#a8b2d8] text-xs mt-0.5">{d.shipment}</p>{d.uploaded&&d.file&&<p className="text-[#6366f1] text-xs mt-1">📎 {d.file}</p>}</div><div className="flex-shrink-0">{d.uploaded?<span className="text-xs bg-green-400/10 text-green-400 px-2 py-1 rounded-full font-semibold">✓ Uploaded</span>:<button onClick={()=>upload(d.id)} className="text-xs px-3 py-1.5 bg-[#6366f1] text-white font-semibold rounded-lg">Upload</button>}</div></div></Card>))}</div>
    </div>
  );
}
...
function PickupPage() {
  ...
  const update = (id,field,value) => setPickups(list=>list.map(p=>p.id===id?{...p,[field]:value}:p));
  const confirm = (id) => setPickups(list=>list.map(p=>p.id===id?{...p,confirmed:true}:p));
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-white">📅 Pickup Scheduling</h1>
      <p className="text-[#a8b2d8] text-sm">Set pickup dates, warehouse locations, and vehicle details.</p>
      <div className="space-y-4">{pickups.map(p=>(<Card key={p.id}><div className="flex items-start justify-between gap-3 mb-3"><div><p className="text-white font-bold text-sm">{p.exporter}</p><p className="text-[#a8b2d8] text-xs">📦 {p.product}</p></div><span className={`text-xs px-2 py-1 rounded-full font-semibold ${p.confirmed?'bg-green-400/10 text-green-400':'bg-yellow-400/10 text-yellow-400'}`}>{p.confirmed?'✓ Confirmed':'Pending'}</span></div>{!p.confirmed?(<div className="space-y-3 border-t border-white/5 pt-3"><div><label className="text-[#a8b2d8] text-xs mb-1 block">Pickup Date</label><input type="date" value={p.pickupDate} onChange={e=>update(p.id,'pickupDate',e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6366f1]" /></div><div><label className="text-[#a8b2d8] text-xs mb-1 block">Warehouse</label><input value={p.warehouse} onChange={e=>update(p.id,'warehouse',e.target.value)} placeholder="e.g. ICD Tughlakabad, Delhi" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6366f1]" /></div><div><label className="text-[#a8b2d8] text-xs mb-1 block">Vehicle Details</label><input value={p.vehicle} onChange={e=>update(p.id,'vehicle',e.target.value)} placeholder="e.g. MH-04-AB-1234 (Tata 407)" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6366f1]" /></div><button onClick={()=>confirm(p.id)} className="w-full py-2.5 bg-[#6366f1] text-white font-bold rounded-xl">Confirm Pickup</button></div>):(<div className="space-y-1.5 border-t border-white/5 pt-3"><div className="flex gap-2"><span className="text-[#a8b2d8] text-xs w-28">Pickup Date:</span><span className="text-white text-xs">{new Date(p.pickupDate).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</span></div><div className="flex gap-2"><span className="text-[#a8b2d8] text-xs w-28">Warehouse:</span><span className="text-white text-xs">{p.warehouse}</span></div><div className="flex gap-2"><span className="text-[#a8b2d8] text-xs w-28">Vehicle:</span><span className="text-white text-xs">{p.vehicle}</span></div></div>)}</Card>))}</div>
      <Card><p className="text-xs text-[#6366f1] font-semibold uppercase tracking-widest mb-3">Live Tracking</p><div className="space-y-2">{[{id:'TRK-20240112',route:'Delhi → London',status:'In Transit',last:'Dubai Hub — Jan 14, 2024'},{id:'TRK-20240110',route:'Chennai → Singapore',status:'Customs',last:'Singapore Port — Jan 13, 2024'}].map(t=>(<div key={t.id} className="bg-white/5 rounded-xl p-3"><div className="flex items-center justify-between mb-1"><span className="text-white font-bold text-sm">{t.id}</span><span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_COLOR[t.status]}`}>{t.status}</span></div><p className="text-[#a8b2d8] text-xs">{t.route}</p><p className="text-[#6366f1] text-xs mt-1">📍 Last update: {t.last}</p></div>))}</div></Card>
    </div>
  );
}
