...
const ChaBookingScreen = () => {
  ...
  const fetchChas = async () => {
    try {
      const r = await api.get('/exporter/cha-directory');
      setChas(r.data?.data || r.data || []);
  ...
  useFocusEffect(useCallback(() => { fetchChas(); fetchShipments(); }, []));

  const bookForShipment = async (shipmentId) => {
    try {
      await api.put(`/exporter/shipments/${shipmentId}`, { assignedChaId: bookModal._id });
      Alert.alert('Booked', 'CHA booked for shipment!');
      setBookModal(null);
      fetchShipments();
    } catch (e) {
      Alert.alert('Error', e?.response?.data?.message || 'Failed to book CHA');
    }
  };
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.headerBlock}>
            <Text style={styles.title}>Book a CHA</Text>
            <Text style={styles.subtitle}>Browse registered customs house agents and book one to handle customs clearance for your shipment.</Text>
          </View>
        }
        ListEmptyComponent={!loading && <EmptyState icon="📑" title="No CHAs" subtitle="No CHAs are registered on the platform yet." />}
        renderItem={({ item }) => (
          <Card>
            <View style={styles.rowTop}>
              <View style={styles.avatar}><Text style={styles.avatarText}>{(item.email || 'C')[0].toUpperCase()}</Text></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.name}>{item.email?.split('@')[0]}</Text>
                {!!item.portCode && <Text style={styles.port}>{item.portCode}</Text>}
              </View>
            </View>
            {!!item.customBrokerLicense && <Text style={styles.meta}>🪪 License: {item.customBrokerLicense}</Text>}
      ...
      <ModalSheet visible={!!bookModal} onClose={() => setBookModal(null)} title={`Book ${bookModal?.email?.split('@')[0] || ''} for Shipment`}>
        {shipments.length === 0 ? (
          ...
          <TouchableOpacity key={s._id} style={styles.shipmentRow} onPress={() => bookForShipment(s._id)}>
            ...
            {(s.assignedChaId?._id || s.assignedChaId) === bookModal?._id && <Text style={styles.assignedTag}>✓ Booked</Text>}
          </TouchableOpacity>
        ))}
      </ModalSheet>
    </View>
...
export default ChaBookingScreen;
